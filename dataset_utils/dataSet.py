...
def convert_q_seq_to_bert_feats(q_seq, seq_length, tokenizer):
    tokens = ["[CLS]"] + tokenizer.tokenize(q_seq)
    tokens.append("[SEP]")
    ...
    while len(bert_ids) < seq_length:
        bert_ids.append(0)
        bert_mask.append(0)
    return np.array(bert_ids, np.int32), np.array(bert_mask, np.int32)
...
class vqa_dataset(Dataset):
    def __init__(self, imdb_file,
        ...
        self.pretrained_bert_dir = data_params['pretrained_bert_dir']
    ...
    def __getitem__(self, idx):
        ...
        if self.pretrained_bert_dir:
            tokenizer = BertTokenizer.from_pretrained(self.pretrained_bert_dir, do_lower_case=True)
            bert_ids, bert_mask = convert_q_seq_to_bert_feats(question_str, self.T_encoder, tokenizer)
        question_inds = (
        ...
        if self.pretrained_bert_dir:
            sample = dict(input_seq_batch=input_seq,
                          bert_ids_batch=bert_ids,
                          bert_mask_batch=bert_mask,
                          seq_length_batch=seq_length)
        else:
            sample = dict(input_seq_batch=input_seq,
                          bert_ids_batch=False,
                          bert_mask_batch=False,
                          seq_length_batch=seq_length)
