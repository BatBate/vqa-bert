...
def prepare_data_set(imdb_file_label, image_dir_label, **data_config):
    ...
    pretrained_bert_dir = data_config['pretrained_bert_dir'] \
        if 'pretrained_bert_dir' in data_config else False
    ...
    assert len(imdb_files) == len(image_feat_dirs), \
        imdb_files + "has different length with " + image_dir_label
