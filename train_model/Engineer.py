...
def one_stage_train(myModel, data_reader_trn, my_optimizer,
                    bert_optimizer, loss_criterion, snapshot_dir, log_dir,
...
def one_stage_run_model(batch, my_model, eval_mode, add_graph=False, log_dir=None):
    ...
    # print("in one_stage_run_model, batch: ", batch)
    input_text_seqs = batch['input_seq_batch']
    bert_ids = batch['bert_ids_batch']
    bert_mask = batch['bert_mask_batch']
    input_images = batch['image_feat_batch']
    is_bert = False
    if type(bert_ids) != type(False):
        bert_ids_variable = Variable(bert_ids.type(torch.LongTensor))
        bert_mask_variable = Variable(bert_mask.type(torch.LongTensor))
        is_bert = True
    ...
    if use_cuda:
        ...
        bert_ids_variable = bert_ids_variable.cuda()
        bert_mask_variable = bert_mask_variable.cuda()
    ...
    logit_res = my_model(input_question_variable=input_txt_variable,
                         is_bert=is_bert,
                         bert_ids_variable=bert_ids_variable,
                         bert_mask_variable=bert_mask_variable,
